    ...
    <div class="container px-4 mx-auto">
        ...
        <div class="flex flex-col gap-4 md:flex-row">
            ...
            <div class="md:w-3/4">
                ...
                <td>
                  <button wire:click='removeItem({{ $item['product_id'] }})' class="px-3 py-1 border-2 rounded-lg bg-slate-300 border-slate-400 hover:bg-red-500 hover:text-white hover:border-red-700"><span wire:loading.remove wire:target='removeItem({{ $item['product_id'] }})'>Remove</span><span
            ...
            <span class="font-semibold">{{ Number::currency($grand_total, 'PHP') }}</span>
                    ...
                    @if ($cart_items)
                        <a href="/checkout" class="bg-blue-500 block text-center text-white py-2 px-4 rounded-lg mt-4 w-full">Checkout</a>                      @endif
                </div>
            </div>
        </div>
    </div>
